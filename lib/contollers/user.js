const UserInfo = require('../mongo_models/users/UserInfo')
...
exports.me = async (req, res) => {
    ...
    try {
        ...
        //check user exist
        const findUser = await await UserInfo.findOne({ appUserID: id }).select("-appUserID");
...
exports.allAppUsers = async (req, res) => {
    //get it from mongodb
    try {
        const allUsers = await UserInfo.find({}).select('-user_password -user_create_by_user_id');
