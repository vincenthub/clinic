...
const StaffInfoModel = require('../mongo_models/users/StaffInfo')
...
const { FAILED_ADD_NEW_USER, TEMPORARY_PASSWORD_EXPIRED, INVALID_CREDENTIAL, ACCOUNT_LOCK } = require('../utilities/ResponseHelper/ResponseDescriptionKeyPair')
        ...
        if(numberOfAttempts >= 0){
            await UserInfoModel.findOneAndUpdate(
                ...
                { 
                    ...
                    staff_last_login_date: moment().tz(process.env.TIMEZONE) 
                },
                { upsert: true }
            )
        }
