...
const { BadResponse, OkResponse, InternalServerErrorResponse, UnprocessableResponse } = require('../utilities/apiResponse');
const { USER_TAKEN, WRONG_CREDENTIALS, ACCOUNT_LOCKED } = require('../utilities/apiResponse/responeCodes');
...
exports.register = async (req, res) => {
    ...
    try {
        ...
        if(!errors.isEmpty()) BadResponse(res, errors.array())
        ...
        if(findUser) UnprocessableResponse(res, USER_TAKEN)    
...
exports.login = async (req, res) => {
    ...
    try {
        ...
        const isPassMatch = findUser && await bycrypt.compare(password, findUser.user_password);

        if(!findUser || findUser.user_create_status !== RECORD_STATUS.ACTIVE) BadResponse(res, WRONG_CREDENTIALS) 

        //get user login attepmts
        const numberOfAttempts = findUser.user_login_attempts || 0;
        const attepmts = 10;

        if(numberOfAttempts >= attepmts) BadResponse(res, ACCOUNT_LOCKED)
        ...
        if(!isPassMatch){
            ...
            BadResponse(res, WRONG_CREDENTIALS);
        }

        if(numberOfAttempts > 0 && numberOfAttempts <= attepmts){
            await UserInfoModel.findOneAndUpdate( 
                { _id: findUser.id }, 
