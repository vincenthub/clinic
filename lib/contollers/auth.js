...
const {HttpError, HttpUnprocessedRequest, HttpInternalServerError, HttpNotFound, HttpBadRequest} = require("../utilities/ResponseHelper/ErrorDetail");
...
exports.register = async (req, res) => {
    ...
    try {
        ...
        if(findUser) throw new HttpBadRequest('AUTH003/2', "Failed to add new user");
        ...
        if (AddressInfo) {
            ...
            if (UserInfo) {
                ...
                jwt.sign(
                    ...
                    (err, token) => {
                        if(err) throw err;
                        res.status(REQUEST_OK).json( SuccessResponse(EndpointKeyPair.register, { token }));
                    }
                )
        ...
        session.endSession();
        res.status(error.statusCode).json( ErrorResponse( EndpointKeyPair.register, error ))
    }
}
...
exports.login = async (req, res) => {
    ...
    try {
        ...
        if(!findUser || findUser.user_create_status !== RECORD_STATUS.ACTIVE) throw new HttpBadRequest('AUTH004/2', "Invalid credential supplied");

        //get user login attepmts
        const numberOfAttempts = findUser.user_login_attempts || 0;
        const attempts = 4;

        if(numberOfAttempts >= attempts) throw new HttpBadRequest('AUTH004/3', "Your account is locked"); 
        ...
        if(!isPassMatch){
            ...
            throw new HttpBadRequest('AUTH004/4', "Invalid credential supplied");
        }
        ...
        jwt.sign(
            ...
            (err, token) => {
                if(err) throw err;
                if(token){
                    res.status(REQUEST_OK).json( SuccessResponse(EndpointKeyPair.login, { token }));
                }
            }
        )

    } catch (error) {
        res.status(error.statusCode).json( ErrorResponse( EndpointKeyPair.login, error ) )
    }
}
