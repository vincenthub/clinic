...
const { AddressInfoModel, UserInfoModel, StaffInfoModel, ClinicInfoModel } = require('../mongo_models')
const UnverifiedContactNumber = require('../mongo_models/UnverifiedContactNumber/UnverifiedContactNumber');
const { sendSMS, ResponseHelper, RECORD_STATUS } = require('../utilities');
...
const { FAILED_ADD_NEW_USER, TEMPORARY_PASSWORD_EXPIRED, INVALID_CREDENTIAL, ACCOUNT_LOCK, STAFF_NO_CLINIC, USER_IS_NOT_STAFF } = require('../utilities/ResponseHelper/ResponseDescriptionKeyPair');
...
exports.login = async (req, res) => {
    ...
    try {
        ...
        const isPassMatch = findUser && await bcrypt.compare(password, findUser.user_password) || false;
...
exports.loginStaff = async (req, res) => {
    ...
    try {
        ...
        if(isTemporaryPassword){
            ...
            // Get timestamp difference from `now`
            const timeDiff = findStaff && Math.round(((new Date(findStaff.staff_password_expiry_date).getTime()) - Date.now()) / 1000)

            //  console.log("TIME DIFFERENCE::" + timeDiff);
            if (findStaff && timeDiff <= 0) throw new HttpUnprocessedRequest('AUTH005/3', TEMPORARY_PASSWORD_EXPIRED);
        ...
        //check if staff is in a clinic
        const findStaffInClinic = findStaff && await ClinicInfoModel.findOne({ clinic_staffs: findStaff.id, clinic_create_status: RECORD_STATUS.ACTIVE });
        
        if(!findStaffInClinic) throw new HttpUnprocessedRequest('AUTH005/6', STAFF_NO_CLINIC);

        //jwt sign
        const  payload = {
            id: findStaff.id,
            clinic_id: findStaffInClinic.id
        }
...
exports.updateUserStatus = async (req, res) => {
    ...
    try {
        ...
        //check user exist
        const findUser = await UserInfoModel.findOne({ _id: id });
