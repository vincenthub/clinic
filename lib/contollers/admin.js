...
exports.createAdmin = async (req,res) => {
    ...
    try {
        ...
        if ( AdminInfo ) {
            ...
            const sentEmail = await sendEmail( email, 'Your new account information', `Your password is ${password}`);
            // TODO: change error as code
            if ( sentEmail !== true ) throw new HttpInternalServerError('ADMIN001/3', 'Email could not be sent')

            return res.status(REQUEST_OK).json( SuccessResponse(EndpointKeyPair.createAdmin, {}));
        }
